    ...
    methods:{
      ...
      banUser(id,status){
        ...
        this.$confirm('此操作将' + ban +'该用户, 是否继续?', '提示', {
          ...
          axios.put(`http://localhost:8201/admin/user/userManager/banUser/${id}/${status}`).then(
      ...
      getList(page = 1) {
        this.current = page
        axios.post(`http://localhost:8201/admin/user/userManager/findPage/${this.current}/${this.limit}`,this.searchObj).then(
          response => {
      ...
      removeDataById(id) {
        this.$confirm('此操作将永久删除该用户信息, 是否继续?', '提示', {
          ...
          axios.delete(`http://localhost:8201/admin/user/userManager/${id}`).then(
          // eslint-disable-next-line no-unused-vars
          response => {
            //  提示
              this.$message({
      ...
      removeRows() {
        this.$confirm('此操作将永久删除用户信息, 是否继续?', '提示', {
          ...
          axios.delete(`http://localhost:8201/admin/user/userManager/batchRemove`,{data: idList}).then(
          // eslint-disable-next-line no-unused-vars
            response => {
              //  提示
              this.$message({
