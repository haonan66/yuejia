<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="邮箱（可不填）" prop="email">
      ...
      <el-form-item label="上传头像图片（可不填）">
        <el-upload
          ...
          action="/user/userManager/upload"
...
export default {
            ...
            validator: (rule, value, callback) => {
              if (!value) {
                ...
                if (uPattern.test(value)) {
                  ...
                  return callback(
                    new Error(
                      "用户名输入不合法，格式为4到16位（字母，数字，下划线，减号）"
                    )
                  );
                }
              }
              ...
              if (!value) {
                ...
                if (value.trim() === "") {
                  ...
                  if (uPattern.test(value)) {
                    ...
                    return callback(new Error("请输入长度在2-20位的昵称"));
                  }
                }
              }
            },
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.user.iconPath =
        "/user/userIcons/" + splicePaths[splicePaths.length - 1];
    },
    ...
    // 根据 id 查询
    getUserById(id) {
      axios.get(`/user/userManager/getUser/${id}`).then(
        (response) => {
    ...
    // 添加
    save() {
      axios.post(`/user/userManager/saveUser`, this.user).then(
    ...
    //  修改
    update() {
      axios.post(`/user/userManager/updateUser`, this.user).then(
        // eslint-disable-next-line no-unused-vars
        (response) => {
          //  提示
          this.$message({
